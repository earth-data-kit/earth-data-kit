...
def discover(fp):
    ...
    futures = []
    with concurrent.futures.ProcessPoolExecutor(max_workers=helpers.get_max_workers()) as executor:
        ...
        uniq_bands = i_df.groupby(by=["description", "x_res", "y_res", "dtype"]).size().reset_index(name="tile_count")
        logger.info ("\n%s", uniq_bands)
