"""
Planetary Computer Test - Sentinel-2
Tests EDK with Sentinel-2 L2A data
"""
...
TEST_CONFIG = {
    ...
    "date_range": (datetime.datetime(2024, 12, 29), datetime.datetime(2024, 12, 30)),  # Recent single day
    "bbox": (-122.45, 37.75, -122.40, 37.80),  # Small bbox in San Francisco
    "dataset_name": "pc-sentinel2-test",
}
...
def _test():
    ...
    output_vrts = sorted([f for f in glob.glob(f"{output_base_vrt}/*.vrt")
                          if not any(x in f for x in ['-B04.', '-B03.', '-B02.', '.tmp.'])])

    assert len(output_vrts) > 0, f"No VRT files found in {output_base_vrt}"

    golden_tar_path = f"{FIXTURES_DIR}/goldens/pc-sentinel2.tar"
    golden_base = f"/vsitar/{golden_tar_path}/pc-sentinel2"
...
def _generated_golden_archives():
    tmp_dir = os.getenv('TMP_DIR', DEFAULT_TMP_DIR)
    with tarfile.open(
        f"{FIXTURES_DIR}/goldens/pc-sentinel2.tar", "w:tar"
    ) as tar:
        tar.add(
            f"{tmp_dir}/{TEST_CONFIG['dataset_name']}/pre-processing",
            arcname="pc-sentinel2",
        )
