...
def transform_bbox(xmin, ymin, xmax, ymax, source_epsg, target_epsg):
    ...
    for i in range(4):
        x_t, y_t = transform_coordinates(x_corners[i], y_corners[i], source_epsg, target_epsg)
        x_transformed.append(x_t)
        y_transformed.append(y_t)

    # Get the min/max of the transformed coordinates
    return min(x_transformed), min(y_transformed), max(x_transformed), max(y_transformed)
